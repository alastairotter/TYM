...
if($status == 1) { $status = "Kept in part"; }
if($status == 2) { $status = "Kept"; }
if($status == 3) { $status = "Broken"; }
if($status == 4) { $status = "In progress"; }
...
if($status) {
    ...
    // get categories
    $sql = "Select distinct category from promises where status = '$status' ";
        $query = mysqli_query($db, $sql);
    $promises = array();
...
//        $row['cat_name'] = $row['category'];
        ...
        $sql = "Select * from promises where status = '$status'";
